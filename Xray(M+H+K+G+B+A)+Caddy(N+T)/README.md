...
1、Caddy 加 caddy-l4 插件定制编译才可以实现 SNI 分流共用 TCP/UDP 端口（含定向 UDP 转发）。
...
4、Caddy 版本不小于 v2.9.0 才支持 SNI 分流共用 TCP/UDP 端口（含定向 UDP 转发）。

5、使用本人 Releases 中编译好的 Caddy 文件，可同时支持 SNI 分流共用 TCP/UDP 端口（含定向 UDP 转发）、NaiveProxy、Trojan-Go 及 PROXY protocol 接收等应用。
...
8、本示例 H、N 均支持 HTTP/3 传输。若想要由 Caddy 处理的 HTTP/3 应用高速传输，建议[增加服务端系统的 UDP 缓冲区大小](https://github.com/quic-go/quic-go/wiki/UDP-Buffer-Sizes)。

9、本示例 K 为选配、与 H 共用 VLESS+XHTTP 配置，仅服务端支持双栈（IPv4 与 IPv6） 推荐配置，使用它与 H 组合可实现 XHTTP 应用上下行分离。
