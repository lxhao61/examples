...
六、V2Ray/Xray 的 SNI 分流配置方法

此方法也可以解决 V2Ray 或 Xray 应用与网站应用（原网站不想做回落网站，或 Nginx、Caddy 等有多个网站应用。）共用 443 端口问题。

注意：

1、V2Ray/Xray 的 SNI 分流不支持 PROXY protocol 发送。

2、1_sni_config.json 采用 Local Loopback 连接，实现 SNI 分流的端口转发。

3、2_sni_config.json 采用 UDS 连接，实现 SNI 分流的进程转发。

4、V2Ray/Xray 的 SNI 分流已被其它 SNI 分流优势替代，其配置示例仅备份及参考等。
