...
5、Caddy 版本不小于 v2.9.0 才支持 SNI 分流共用 TCP/UDP 端口（含定向 UDP 转发）。

6、使用本人 Releases 中编译好的 Caddy 文件，可同时支持 SNI 分流共用 TCP/UDP 端口（含定向 UDP 转发）、NaiveProxy 及 PROXY protocol 等应用。
